...
async function connectDatabase() {
    try {
        ...
        if (config_1.config.databaseUrl.includes('supabase.co')) {
            // Initialize Supabase client
            supabaseClient = (0, supabase_js_1.createClient)(config_1.config.supabaseUrl, config_1.config.supabaseServiceKey);
            ...
            logger_1.logger.info('Supabase client connected successfully');
        }
        else {
            pool = new pg_1.Pool({
            ...
            client.release();
            logger_1.logger.info('Database connection pool created successfully');
            // Handle pool errors
            pool.on('error', (err) => {
...
async function query(text, params) {
    ...
    try {
        if (isUsingSupabase()) {
            ...
            logger_1.logger.debug('Supabase query executed', {
...
function initializeSupabase() {
    if (!config_1.config.useSupabase) {
    ...
    supabaseClient = (0, supabase_js_1.createClient)(config_1.config.supabaseUrl, config_1.config.supabaseAnonKey);
...
function isUsingSupabase() {
    return config_1.config.databaseUrl.includes('supabase.co');
}
/**
...
async function cleanupOldData() {
    try {
        const result = await query('DELETE FROM analytics WHERE timestamp < NOW() - INTERVAL \'${retentionDays} days\'', [config_1.config.retentionDays]);
        logger_1.logger.info(`Cleaned up ${result.rowCount} old analytics records`);
    }
    catch (error) {
